...
class simplex_dynamics:
    ...
    def calc_direction_and_strength(self):
        direction= [self.f(self.xy2ba(x,y)+.001,0) for x,y in zip(self.trimesh.x, self.trimesh.y)]
        ...
        self.pvals =[np.linalg.norm(v)+.0001 for v in direction]
    ...
    def plot_simplex(self,ax,cmap='viridis',typelabels=["A","B","C"],**kwargs):
        ...
        ax.tricontourf(self.trimesh, self.pvals, norm=colors.LogNorm(vmin=np.array(self.pvals).min(), vmax=np.array(self.pvals).max()),alpha=0.8,cmap=cmap,**kwargs)
