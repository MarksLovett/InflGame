...
class IQL_sync():
    ...
    def action_choice(self,episode):
        action_dict = {}
        ...
        for player in self.env.possible_agents:
    ...
    def Q_step(self,episode):
        ...
        for player in self.env.possible_agents:
            self.Q_table[player][previous_observations[player]][action_dict[player]] = (1-self.alpha) *self.Q_table[player][previous_observations[player]][action_dict[player]] + self.alpha*(self.rewards[player] + self.gamma * self.Q_table[player][self.observations[player]][self.Q_max_action(player)])
