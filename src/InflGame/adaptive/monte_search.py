"""
.. module:: monte_search
   :synopsis: Provides equilibrium search and analysis tools for adaptive environments in influencer games.


Monte Carlo Equilibrium Search Module
======================================

This module provides tools for searching and analyzing equilibria in adaptive game environments
using grid search and Monte Carlo methods. It includes functionality for detecting convergence,
identifying limit cycles, classifying equilibrium types, and visualizing the distribution of
equilibria across the strategy space.

The module is designed to work with the `AdaptiveEnv` class and provides comprehensive analysis
of equilibrium structures in multi-agent influencer games.
...
Usage:
------
The `search_env` class can be used to search for equilibria across the strategy space using either
...
class search_env:
    """
    The search_env class provides a framework for searching and analyzing equilibria in adaptive
    game environments. It supports grid-based and Monte Carlo sampling methods to explore the
    strategy space, detect convergence patterns, identify limit cycles, and classify equilibrium
    types based on agent positions and clustering.
    
    This class is particularly useful for understanding the multiplicity and stability of equilibria
    in multi-agent influencer games across various domains (1D, 2D, and simplex).
    ...
    **Key Features**:
    ...
    def setup_adaptive_env(self) -> None:
        ...
        This method initializes the gradient function environment with the provided
        parameters, enabling gradient ascent dynamics for equilibrium discovery.
        Must be called before performing any equilibrium search operations.
        ...
        **Example**:
        ...
        .. code-block:: python
            ...
            searcher = search_env(...)
            searcher.setup_adaptive_env()
            results = searcher.search_Eq(resolution=10)
        """
    ...
    def unit_cube_3d(self, resolution: int = 10) -> torch.Tensor:
        """
    ...
    def detect_approximate_cycles(self, 
                                   sequence: np.ndarray,
        ...
        This method identifies limit cycles in the dynamics by searching for repeating
        patterns in the position sequence, allowing for some numerical tolerance to
        handle cases where cycles aren't perfectly identical.
    ...
    def _process_grid_point_worker(self, args: Tuple) -> Tuple[str, Dict]:
        """
        Worker function for parallel processing of grid search equilibrium points.
        
        This internal method is called by the parallel processing framework to analyze
    ...
    def search_Eq(self,
        ...
        This method performs a comprehensive search for equilibria using either grid-based
        sampling or Monte Carlo random sampling. It analyzes convergence behavior from each
        starting point, detecting both stable equilibria and limit cycles.
        
        **Search Methods**:
        
        - **Grid Search** (``use_monte_carlo=False``): Systematic sampling on a uniform grid
        - **Monte Carlo** (``use_monte_carlo=True``): Random sampling from uniform distribution
        
        **Example**:
        
        .. code-block:: python
            
            # Grid search with 10 points per dimension
            results = searcher.search_Eq(
                resolution=10,
                time_steps=10000,
                use_parallel=True
            ...
            # Monte Carlo search with 1000 random samples
            results = searcher.search_Eq(
                number_samples=1000,
                time_steps=10000,
    ...
    def analyze_grid_search_results(self,
                                     results_dict: Optional[Dict] = None,
                                     position_tolerance: float = 1e-3) -> Dict:
        """
        Analyze equilibrium search results to classify equilibrium types and compute statistics.
        
        This method processes the results from `search_Eq()` to categorize convergence behavior,
        classify equilibrium types based on agent clustering, and compute comprehensive statistics
        about the equilibrium landscape.
    ...
    def classify_equilibrium_type(self,
                                   positions: np.ndarray,
                                   tolerance: float = 1e-3) -> str:
        """
        Classify equilibrium type based on agent clustering and spatial arrangement.
        
        This method analyzes the final positions of agents to determine the equilibrium
        structure, grouping agents that are within tolerance of each other and providing
        a descriptive classification that captures both the clustering pattern and spatial
        ordering.
        
        **Classification Notation**:
        
        - ``(n)``: All n agents clustered together
        - ``(n-1,1)``: n-1 agents at lower position, 1 isolated higher
        - ``(1,n-1)``: 1 agent at lower position, n-1 grouped higher
        - ``c(1,n-2,1)``: Symmetric arrangement with middle group centered at 0.5
        - ``l(1,n-2,1)``: Asymmetric with middle group below 0.5
        - ``u(1,n-2,1)``: Asymmetric with middle group above 0.5
        
        **Example**:
        
        .. code-block:: python
            
            positions = np.array([0.2, 0.5, 0.5, 0.8])
    ...
    def print_results_summary(self) -> None:
        ...
        This method displays a comprehensive, human-readable summary of the equilibrium
        search results, including convergence statistics, equilibrium type distributions,
        and detailed breakdowns of each equilibrium category.
