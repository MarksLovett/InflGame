...
.. template taken from Pandas and scipy
...
The InflGame package lets you model influencer games via adaptive dynamics (gradient ascent along the reward function) and MARL (multi agent reinforcement learning.) Through these methods 
players maximize their reward. In symmetric games this leds to a symmetric Nash under certian circumstances. 

Influencer Games
=========================
The main focus of this package is to provide a framework to studying the dynamics of spatial influence in a multi-player resource competition game. The package takes two paths to study influence games

1. Adaptive dynamics (gradient ascent of the reward vectors)
   
   - By following the gradient of the rewards available to a player they can move toward a maxima value for their return
...
Given a discrete resource distribution :math:`u_i` has the following form
...
Where :math:`G_{i}(x_i,x_{-i},b)` is the probability of influence and is the ratio of a players influence over a resource point to the sum of all influence on that point. The probability of influence is defined as 

.. math:: 
    G_i(x_i,x_{-i},b)= \frac{f_{i}(x_i,b)}{\sum_{j\in I} f_{j}(x_j,b)},


where :math:`f_{i}(x_i,b)` is the :math:`i` th players influence over a resource point :math:`b` .
